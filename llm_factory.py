...
def get_llm(
    ...
    **kwargs
    ...
    cbs = [PromptDebugCallback()] if os.getenv("DEBUG_PRINT_PROMPT", "").lower() == "true" else []
    ...
    if provider == "openai":
        ...
        return ChatOpenAI(model=model_name or "gpt-4o", temperature=temperature,
                          api_key=os.getenv("OPENAI_API_KEY"), callbacks=cbs or None, **extra, **kwargs)
    elif provider == "azure":
        extra = {"use_responses_api": True} if use_responses_api else {}
        ...
        return ChatGoogleGenerativeAI(model=model_name or "gemini-1.5-pro", temperature=temperature,
                                      google_api_key=os.getenv("GOOGLE_API_KEY"), callbacks=cbs or None, **kwargs)
    elif provider == "ollama":
        return ChatOllama(model=model_name or "llama3", temperature=temperature,
